...
def FireModule(s_1x1, e_1x1, e_3x3, name):
    ...
    def layer(x):
        squeeze = Conv2D(s_1x1,(1, 1), activation='relu', kernel_initializer='glorot_uniform', name=name+'/squeeze1x1')(x)
        squeeze = BatchNormalization(name=name+'/squeeze1x1_bn')(squeeze)

        # Needed to merge layers expand_1x1 and expand_3x3.
        expand_1x1 = Conv2D(e_1x1,(1, 1), activation='relu', kernel_initializer='glorot_uniform', name=name+'/expand1x1')(squeeze)
        ...
        expand_3x3 = Conv2D(e_3x3,(3, 3), padding='same', activation='relu', kernel_initializer='glorot_uniform', name=name+'/expand3x3')(squeeze)
...
def SqueezeNet(nb_classes, rows=224, cols=224, channels=3):
    """
    ...
    conv1 = Conv2D(64,(3, 3), activation='relu', strides=(2, 2), kernel_initializer='glorot_uniform', name='conv1')(input_image)
    ...
    dropout9 = Dropout(rate=0.5, name='dropout9')(fire9)
    # conv10 output shape = (?, 27, 27, nb_classes)
    conv10 = Conv2D(nb_classes,(1, 1), activation='relu', kernel_initializer='he_normal', name='conv10')(dropout9)
